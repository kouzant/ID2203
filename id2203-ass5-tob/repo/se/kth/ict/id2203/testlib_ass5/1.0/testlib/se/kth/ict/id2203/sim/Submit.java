...
public class Submit
{
    public static String submit(final String email, final String password, final String assignment) {
        return submit(email, password, assignment, "not_supplied");
    }
    
    public static String submit(final String email, final String password, final String assignment, final String svs) {
        String responseLine;
        try {
            final URL url = new URL("http://cloud7.sics.se:11700/submit");
            final HttpURLConnection con = (HttpURLConnection)url.openConnection();
            final String urlParameters = String.format("email=%s&password=%s&assignment=%s&abc=404&svs=%s", URLEncoder.encode(email, "UTF-8"), password, assignment, svs);
